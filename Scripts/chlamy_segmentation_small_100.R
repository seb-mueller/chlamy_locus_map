...
mygap <- 100
comment <- "multi200_wt_adrian"

## taking out SL2362-73 and SL2126-2137
# they are anntated in the csv file
meta <- meta %>% 
  #   filter(InCurrentLociRun == "Yes")
  # filter(InCurrentLociRun_Safe2 == "Yes")
  filter(GenuineControls == "wt") %>%
  filter(Author == "Adrian Valli")
...
# save(aD, file=file.path(datadir ,"aD_chlamy_segmentation.RData"))
 # load(file=file.path(datadir ,"aD_chlamy_segmentation.RData"))
...
# [1]   1   5  35 179 994
aD<-aD[aD@alignments$multireads<200]
...
#Process alignment data to find potential segements
 ...
 save(sD, file=file.path(datadir ,paste0("sD_chlamy_segmentation_",comment,mygap,".RData")))
#load(file=file.path(datadir ,paste0("sD_chlamy_segmentation_",comment,mygap,".RData")))

#Generate Locus map
hS<-heuristicSeg(sD=sD,aD=aD,getLikes=TRUE,cl=cl)
save(hS, file=file.path(datadir, paste0("hS_chlamy_segmentation", comment, mygap, ".RData")))

#Generate a genome map
segD<-classifySeg(aD=aD,sD=sD,cD=hS, getLikes=TRUE,cl=cl)
save(segD, file=file.path(datadir ,paste0("segD_chlamy_segmentation", comment, mygap, ".RData")))
