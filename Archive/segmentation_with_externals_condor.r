...
save(sD, file=file.path(datadir ,paste0("sD_chlamy_segmentation_",mygap,".RData")))
#load("sDlt20_first_chlamy_segmentation_nick.RData")

#Generate Locus map
hS<-heuristicSeg(sD=sD,aD=aD,getLikes=TRUE,cl=cl)
save(hS, file=file.path(datadir ,paste0("hS_chlamy_segmentation",mygap,".RData")))

#Generate a genome map
segD<-classifySeg(aD=aD,sD=sD,cD=hS, getLikes=TRUE,cl=cl)
save(segD, file=file.path(datadir ,paste0("segD_chlamy_segmentation",mygap,".RData")))
